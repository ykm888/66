...
/* 物理核心：坐标锁定 1MB (0x100000)。必须与 filogic.mk 中的 seek=1024 严格物理对齐 */
#define FIP_BASE			0x100000

/* 物理核心：载入空间预留 2MB (0x200000)。防止 U-Boot 镜像体积过大导致读取截断 */
#define FIP_SIZE			0x200000

/* 物理核心：MT7981 默认 MPLL 分频时钟逻辑锁定 */
#define MTK_QSPI_SRC_CLK		CB_MPLL_D2

/**
 * mtk_plat_qspi_init: 初始化物理链路
 * 负责打通 CPU 到 SPI-NOR 闪存的物理引脚通路
 */
int mtk_plat_qspi_init(void)
{
	/* 物理引脚对齐：初始化 SPI 硬件接口引脚复用 (锁定 SPIM2 组) */
 ...
 * 告知 BL2 逻辑：U-Boot (FIP) 存放在 Flash 的具体物理位置
 */
