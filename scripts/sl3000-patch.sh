...
# 2. 彻底解决 U-Boot 源码缺失及新版 2024.10 依赖丢失问题：物理劫持 Makefile
...
if [ -f "$UBOOT_MAKEFILE" ]; then
    ...
    # 物理手术：不仅强制下载替换 defconfig，同时物理追加新版 U-Boot 必备的依赖开关
...
\techo "CONFIG_SYSCON=y" >> $(PKG_BUILD_DIR)/configs/mt7981_emmc_defconfig;' "$UBOOT_MAKEFILE"
    echo "U-Boot Makefile physically hooked with regmap/syscon dependencies."
fi

# 3. 彻底解决内核符号断点：物理 sed 注入
