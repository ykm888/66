...
echo "执行仓库源最高级别物理修复：锁定 SL-3000 并解决内核 Kconfig 交互死锁"
...
# 6. 【核心修复】针对 MTD_OF_PARTS_AIROHA 交互死锁进行物理注入
# 强制开启内核分区支持，防止 syncconfig 弹出 NEW 询问
...
if make defconfig > /dev/null 2>&1; then
    echo "================================================="
    echo "物理审计成功：SL-3000 架构锁定，内核死锁已解除！"
    echo "================================================="
else
    echo "致命错误：配置锁定失败。"
    exit 1
